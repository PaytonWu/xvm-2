...
struct xreward_property_param_t {
    ...
    std::map<common::xcluster_address_t, xstake::cluster_workload_t> validator_workloads_detail;
    std::map<common::xaccount_address_t, std::map<common::xaccount_address_t, uint64_t>> votes_detail;
    xstake::xaccumulated_reward_record accumulated_reward_record;
...
class xtop_zec_reward_contract_new : public xbasic_system_contract_t {
    using xbase_t = xbasic_system_contract_t;

public:
    xtop_zec_reward_contract_new() = default;
    xtop_zec_reward_contract_new(xtop_zec_reward_contract_new const &) = delete;
...
private:
     ...
     * @return bool
     */
    bool reward_is_expire(const common::xlogic_time_t current_time, const xstake::xactivation_record & activation_record) const;
    ...
    bool reward_is_expire_internal(const common::xlogic_time_t current_time,
                                   ...
                                   const xstake::xactivation_record & activation_record,
     ...
     * @return void
     */
    void update_accumulated_record(const xstake::xaccumulated_reward_record & record);
     ...
     */
    void update_accumulated_issuance(const common::xlogic_time_t timer_round, const uint64_t issuance, const xstake::xactivation_record & activation);
     ...
     * @param issue_detail
     */
    void update_issuance_detail(xstake::xissue_detail const & issue_detail);
    ...
    void update_property(const common::xlogic_time_t current_time,
                         ...
                         const xstake::xactivation_record & activation,
                         const xstake::xaccumulated_reward_record & accumulated_reward,
                         xstake::xissue_detail const & issue_detail);
    ...
    std::map<common::xaccount_address_t, uint64_t> calc_votes(std::map<common::xaccount_address_t, std::map<common::xaccount_address_t, uint64_t>> const & votes_detail,
                                                              std::map<common::xaccount_address_t, xstake::xreg_node_info> const & map_nodes);
    ...
    top::xstake::uint128_t calc_zero_workload_reward(bool is_auditor,
                                                     std::map<common::xcluster_address_t, xstake::cluster_workload_t> & workloads_detail,
     ...
     */
    void calc_edge_workload_rewards(xstake::xreg_node_info const & node,
     ...
     * @param reward_to_self node self reward
     */
    void calc_validator_workload_rewards(xstake::xreg_node_info const & node,
                                         std::vector<uint32_t> const & validator_num,
                                         std::map<common::xcluster_address_t, xstake::cluster_workload_t> const & validator_workloads_detail,
     ...
     * @param reward_to_self node self reward
     */
    void calc_auditor_workload_rewards(xstake::xreg_node_info const & node,
                                       std::vector<uint32_t> const & auditor_num,
                                       std::map<common::xcluster_address_t, xstake::cluster_workload_t> const & auditor_workloads_detail,
     ...
     * @param reward_to_self node self reward
     */
    void calc_vote_reward(xstake::xreg_node_info const & node,
     ...
     * @return record_str
     */
    std::string accumulated_reward_serialize(xstake::xaccumulated_reward_record const & record) const;
     ...
     * @return record
     */
    xstake::xaccumulated_reward_record accumulated_reward_deserialize(const std::string & accumulated_reward_serialize) const;
    ...
    contract_common::properties::xmap_property_t<std::string, std::string> m_reward_task{xstake::XPORPERTY_CONTRACT_TASK_KEY, this};
    // string property
    ...
    contract_common::properties::xstring_property_t m_last_read_rec_reg_contract_height{xstake::XPROPERTY_LAST_READ_REC_REG_CONTRACT_BLOCK_HEIGHT, this};
    contract_common::properties::xstring_property_t m_last_read_rec_reg_contract_time{xstake::XPROPERTY_LAST_READ_REC_REG_CONTRACT_LOGIC_TIME, this};
};
using xzec_reward_contract_new_t = xtop_zec_reward_contract_new;
